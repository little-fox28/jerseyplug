...
$socials = [
	'facebook'  => [
		...
		'icon'  => '<svg class="w-10 h-10" viewBox="0 0 24 24" aria-hidden="true" fill="currentColor"><path d="M13.5 9H16V6h-2.2C11.5 6 10 7.5 10 9.7V12H8v3h2v6h3v-6h2.4l.6-3H13V9.8c0-.5.3-.8.8-.8Z" /></svg>',
		'label' => __('Facebook', 'jerseyplug'),
		'color' => 'bg-[#1877F2] hover:bg-[#1877F2]/90', // Facebook Blue
	],
	'instagram' => [
		'url'   => $get_setting('jerseyplug_social_instagram', ''),
		'icon'  => '<svg class="w-10 h-10" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="5"></rect><circle cx="12" cy="12" r="4"></circle><circle cx="17" cy="7" r="1.5" fill="currentColor" stroke="none"></circle></svg>',
	...
	<?php foreach ($socials as $key => $social) : ?>
		<?php if (!empty($social['url'])) : ?>
			<a
				...
				class="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full text-white shadow-lg shadow-black/20 hover:scale-110 transition-all duration-300 <?php echo esc_attr($social['color']); ?>">

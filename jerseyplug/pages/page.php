	...
	<?php if (have_posts()) : ?>
		<?php while (have_posts()) : the_post(); ?>
			<article id="post-<?php the_ID(); ?>" <?php post_class('container mx-auto px-4 max-w-5xl'); ?>>
				<?php if (! (function_exists('is_account_page') && is_account_page()) && ! (function_exists('is_cart') && is_cart()) && ! (function_exists('is_wc_endpoint_url') && is_wc_endpoint_url('order-received'))) : ?>

...
<div class="cart_totals !w-full !float-none bg-gray-50 rounded-2xl p-6 lg:p-8 border border-gray-200 shadow-sm <?php echo (WC()->customer->has_calculated_shipping()) ? 'calculated_shipping' : ''; ?>">
	...
	<h2 class="text-xl font-bold text-[#163300] mb-6"><?php echo esc_html( jerseyplug_pll( 'Order Summary' ) ); ?></h2>
	...
	<table cellspacing="0" class="w-full text-sm">
		...
		<tr class="cart-subtotal border-b border-gray-200/60">
			<th class="py-3 text-left font-normal text-gray-500 !bg-transparent "><?php echo esc_html( jerseyplug_pll( 'Subtotal' ) ); ?></th>
			<td class="py-3 text-right font-bold text-[#163300] !bg-transparent " data-title="<?php echo esc_attr( jerseyplug_pll( 'Subtotal' ) ); ?>"><?php wc_cart_totals_subtotal_html(); ?></td>
		</tr>
			...
			<tr class="cart-discount border-b border-gray-200/60 coupon-<?php echo esc_attr(sanitize_title($code)); ?>">
				...
				<td class="py-3 text-right font-bold text-[#163300] !bg-transparent " data-title="<?php echo esc_attr(wc_cart_totals_coupon_label($coupon, false)); ?>">
					<div class="flex items-center justify-end gap-2">
						...
						$remove_text = jerseyplug_pll( '[Remove]' );
		...
		<?php if (WC()->cart->needs_shipping() && WC()->cart->show_shipping()) : ?>
			...
			<tr class="shipping border-b border-gray-200/60">
				<th class="py-3 text-left font-normal text-gray-500 !bg-transparent "><?php echo esc_html( jerseyplug_pll( 'Shipping' ) ); ?></th>
				<td class="py-3 text-right font-bold text-[#163300] !bg-transparent  [&>ul]:m-0 [&>ul]:p-0 [&>ul>li]:m-0 [&>ul>li]:p-0 [&>ul>li]:list-none" data-title="<?php echo esc_attr( jerseyplug_pll( 'Shipping' ) ); ?>">
		...
		<?php elseif (WC()->cart->needs_shipping() && 'yes' === get_option('woocommerce_enable_shipping_calc')) : ?>
			<tr class="shipping border-b border-gray-200/60">
				<th class="py-3 text-left font-normal text-gray-500 !bg-transparent "><?php echo esc_html( jerseyplug_pll( 'Shipping' ) ); ?></th>
				<td class="py-3 text-right font-bold text-[#163300] !bg-transparent " data-title="<?php echo esc_attr( jerseyplug_pll( 'Shipping' ) ); ?>"><?php woocommerce_shipping_calculator(); ?></td>
			</tr>
		<?php endif; ?>
		...
		if (wc_tax_enabled() && ! WC()->cart->display_prices_including_tax()) {
			...
			if (WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping()) {
				$estimated_text = sprintf(' <small>' . esc_html( jerseyplug_pll( '(estimated for %s)' ) ) . '</small>', WC()->countries->estimated_for_prefix($taxable_address[0]) . WC()->countries->countries[$taxable_address[0]]);
			}
		...
		<tr class="order-total">
			<th class="py-6 text-left font-bold text-gray-900 text-lg !bg-transparent !p-0 !pt-6"><?php echo esc_html( jerseyplug_pll( 'Total' ) ); ?></th>
			<td class="py-6 text-right font-black text-2xl text-[#163300] !bg-transparent !p-0 !pt-6" data-title="<?php echo esc_attr( jerseyplug_pll( 'Total' ) ); ?>"><?php wc_cart_totals_order_total_html(); ?></td>
		</tr>
	...
	<?php if (wc_coupons_enabled()) { ?>
		<div class="coupon mt-8 mb-4 bg-white !rounded-2xl border border-gray-200 p-4">
			<label for="coupon_code" class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3"><?php echo esc_html( jerseyplug_pll( 'Coupon Code' ) ); ?></label>
			<div class="flex gap-2">
				<input type="text" name="coupon_code" class="input-text !w-full !rounded-xl !border border-gray-200 bg-white px-4 py-2.5 text-sm font-bold text-[#163300] placeholder:text-gray-300 focus:border-[#65cf21] focus:outline-none transition-colors" id="coupon_code" value="" placeholder="<?php echo esc_attr( jerseyplug_pll( 'Enter code' ) ); ?>" />
				<button type="submit" class="button shrink-0 flex items-center justify-center !w-auto h-auto px-6 !rounded-xl !bg-[#163300] text-sm font-bold !text-white transition-colors hover:!bg-[#0a1700]" name="apply_coupon" value="<?php echo esc_attr( jerseyplug_pll( 'Apply' ) ); ?>"><?php echo esc_html( jerseyplug_pll( 'Apply' ) ); ?></button>
			</div>
	...
	<div class="mt-4 flex flex-col gap-3 relative z-10">
		...
		<a href="<?php echo esc_url(wc_get_checkout_url()); ?>" class="w-full flex items-center justify-center gap-2 bg-[#163300] !text-white font-bold py-4 rounded-xl shadow-lg hover:bg-[#0a1700] transition-colors group">
			<?php echo esc_html(jerseyplug_pll('Proceed to Checkout')); ?>
	...
	<div class="mt-8 text-center pt-6 border-t border-gray-200/60">
		<span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-4"><?php echo esc_html( jerseyplug_pll( 'Guaranteed Safe Checkout' ) ); ?></span>

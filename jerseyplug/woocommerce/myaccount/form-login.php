				...
				<div x-show="view === 'login'" 
					...
					<form class="woocommerce-form woocommerce-form-login login space-y-5" method="post">
						...
						<div>
							...
							<div class="relative">
								<input class="woocommerce-Input woocommerce-Input--text input-text w-full px-5 py-3.5 pr-12 rounded-xl border-2 border-gray-100 bg-gray-50 focus:bg-white focus:border-[#65cf21] focus:ring-0 transition-all outline-none text-gray-800 font-medium" :type="showLoginPass ? 'text' : 'password'" name="password" id="password" autocomplete="current-password" required />
						...
						<div class="flex items-center justify-between pt-2">
							<label class="woocommerce-form__label woocommerce-form__label-for-checkbox woocommerce-form-login__rememberme inline-flex items-center cursor-pointer group">
								<div class="relative flex items-center justify-center w-5 h-5 border-2 border-gray-300 rounded focus-within:border-[#65cf21] focus-within:ring-2 focus-within:ring-[#65cf21]/30 group-hover:border-[#65cf21] transition-all">
									<input class="woocommerce-form__input woocommerce-form__input-checkbox absolute opacity-0 w-full h-full cursor-pointer peer" name="rememberme" type="checkbox" id="rememberme" value="forever" />
									<svg class="w-3.5 h-3.5 text-[#65cf21] opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
								</div>
					...
					<form method="post" class="woocommerce-form woocommerce-form-register register space-y-5" <?php do_action( 'woocommerce_register_form_tag' ); ?> x-data="{ email: '<?php echo esc_js( ! empty( $_POST['email'] ) ? wp_unslash( $_POST['email'] ) : '' ); ?>' }">
						...
						<?php if ( 'no' === get_option( 'woocommerce_registration_generate_password' ) ) : ?>
							<div>
								...
								<div class="relative">
									<input :type="showRegPass ? 'text' : 'password'" class="woocommerce-Input woocommerce-Input--text input-text w-full px-5 py-3.5 pr-12 rounded-xl border-2 border-gray-100 bg-gray-50 focus:bg-white focus:border-[#65cf21] focus:ring-0 transition-all outline-none text-gray-800 font-medium" name="password" id="reg_password" autocomplete="new-password" required />
